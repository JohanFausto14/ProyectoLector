...
function GraficaAvance({ libros }: { libros: LibroAlumno[] }) {
  ...
  return (
    <div
      ...
      style={{ maxHeight: 380, scrollbarWidth: 'thin', scrollbarColor: '#d4af3760 transparent' }}
    >
      {sorted.map((libro, i) => {
        const pct   = libro.progresoPorcentaje;
        const done  = pct >= 100;
        const fresh = pct === 0;
        const color = done ? '#4caf50' : fresh ? '#a1887f' : '#d4af37';
          ...
          >
            ...
            <div className="flex items-start justify-between gap-2 mb-2">
              <div className="min-w-0">
                <p className="text-sm font-bold text-[#2b1b17] leading-snug truncate">{libro.titulo}</p>
                <p className="text-[11px] text-[#a1887f] mt-0.5 truncate">
...
export default function StatsPage() {
  ...
  const barData = libros
    ...
    .map(l => ({
      ...
      color: l.progresoPorcentaje >= 100 ? '#4caf50' : l.progresoPorcentaje > 0 ? '#d4af37' : '#a1887f',
    }));
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      {/* Tabs */}
      <div className="flex gap-2 border-b border-[#e3dac9]">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl shadow-md border border-[#e3dac9]/50 p-6">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h3 className="font-playfair text-lg font-bold text-[#2b1b17]">Avance de Lectura</h3>
                  <p className="text-xs text-[#a1887f] font-lora italic">Porcentaje completado por libro</p>
                </div>
                <div className="flex gap-3 text-[10px]">
                  {[['#d4af37','En progreso'],['#4caf50','Completo'],['#a1887f','Sin iniciar']].map(([c,l]) => (
                    <div key={l} className="flex items-center gap-1">
                      <div className="w-2 h-2 rounded-full" style={{ background: c as string }} />
                      <span className="text-[#8d6e3f] font-medium">{l}</span>
                    </div>
                  ))}
                </div>
              </div>
              {libros.length === 0
                ? <p className="text-center text-[#a1887f] text-sm py-12">Sin libros asignados aún</p>
                : <GraficaAvance libros={libros} />
              }
            </div>

            {/* Racha de lectura */}
            <div className="bg-white rounded-2xl shadow-md border border-[#e3dac9]/50 p-6 flex flex-col">
              <h3 className="font-playfair text-lg font-bold text-[#2b1b17] mb-1">Tu Racha</h3>
              <p className="text-xs text-[#a1887f] font-lora italic mb-4">Días consecutivos leyendo</p>
              ...
              <div className="flex flex-col items-center flex-1 justify-center gap-2">
                <div className="relative" style={{ width: 148, height: 148 }}>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-2xl font-playfair font-black text-[#2b1b17] leading-none">{stats.rachaActualDias}</span>
                    <span className="text-[10px] text-[#a1887f] uppercase tracking-wider">días</span>
                  </div>
                </div>

                <div className="w-full mt-2 space-y-2">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-[#a1887f]">Racha actual</span>
                  ...
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-[#a1887f]">Mejor racha</span>
                    <span className="font-black text-[#2b1b17]">{stats.rachaMaximaDias}d</span>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-md border border-[#e3dac9]/50 p-6">
              <h3 className="font-playfair text-lg font-bold text-[#2b1b17] mb-5">Rendimiento Académico</h3>
                ...
                {[
                  ...
                  <div key={m.label}>
                    <div className="flex justify-between items-center mb-1.5">
                      <span className="text-sm font-medium text-[#2b1b17]">{m.label}</span>
            ...
            <div className="bg-white rounded-2xl shadow-md border border-[#e3dac9]/50 p-6">
              <div className="flex items-center justify-between mb-5">
                <h3 className="font-playfair text-lg font-bold text-[#2b1b17]">Mi Biblioteca</h3>
                {stats.ultimaActividad && (
                  <span className="text-[11px] text-[#a1887f] font-lora italic">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { label: 'En progreso', value: librosEnProgreso.length, color: '#d4af37',
                  ...
                  <div key={item.label}
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-[#8d6e3f] mt-0.5 leading-tight">{item.label}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {tabActiva === 'libros' && (
        <div className="space-y-3">
          {libros.length === 0 ? (
            <div className="text-center py-16 text-[#a1887f] font-lora italic">No tienes libros asignados aún.</div>
          ) : (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <h4 className="font-playfair font-bold text-[#2b1b17] text-base truncate">{libro.titulo}</h4>
                        <span className={`shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full border ${badgeCls}`}>{badge}</span>
                      </div>
                    ...
                    <div className="flex flex-col items-center shrink-0">
                      <span className="text-3xl font-playfair font-bold" style={{ color }}>{libro.progresoPorcentaje}%</span>
                      {libro.ultimaLectura && (
                        <span className="text-[10px] text-[#a1887f] mt-1 whitespace-nowrap">
                          {new Date(libro.ultimaLectura).toLocaleDateString('es-MX', { day: 'numeric', month: 'short' })}
                        </span>
                      )}
                    </div>
                  </div>
